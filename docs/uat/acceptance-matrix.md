...
| US-A-20 | Learner activity reporting (progress, completion, last access) | `lib/admin/analytics.ts`, `app/admin/analytics`, CSV export | `analytics.test.ts` | PASS (local) | Per-learner enrolment/progress/completion/last-access rows + RFC-4180 CSV (no Clerk id / grading / answers / storage path); admin-guarded. Browser evidence added by the product vertical (analytics section). |
