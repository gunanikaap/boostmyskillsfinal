...
| US-L-05 | Password reset | Clerk-hosted recovery | live browser | PASS (local)* | REAL forgot-password flow completed on the Clerk **Development** instance: new password set, subsequent login succeeded, reached `/dashboard`, `/admin` still rendered, same `app_users` row + admin role. **Method note (*):** the dev instance uses email **CODE**, whereas the strict AC expects a reset **LINK** — recorded here as an **accepted functional equivalent** (both are email-verified secure resets); link parity is a Clerk instance toggle if strictness is required. Production email = B-EMAIL. |
