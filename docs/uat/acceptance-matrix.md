...
| US-L-12 | MCQ score / pass | `lib/player/grade.ts`, `submitMcqAttempt` | `assessment.test.ts`, `assessment-answer-key-privacy.test.ts` | PASS | Server-side scoring, one-attempt, idempotent double-submit. The learner sees their score, the configured pass mark, passed/failed and their own submitted answers; the learner is NOT given the server answer key (FCX-P1-002 — `grading_snapshot` is not selected in learner queries and no correct-option map reaches the client). |
